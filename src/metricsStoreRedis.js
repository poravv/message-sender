/**
 * MetricsStore - Redis Implementation (Legacy)
...
function contactsKey(userId) { return `ms:contacts:${userId}`; }
function contactIdMapKey(userId) { return `ms:contacts:idmap:${userId}`; }
function campaignSeqKey() { return 'ms:campaign:seq'; }
function campaignDataKey(campaignId) { return `ms:campaign:data:${campaignId}`; }
function campaignRecipientsKey(campaignId) { return `ms:campaign:recipients:${campaignId}`; }
function userCampaignsKey(userId) { return `ms:campaigns:${userId}`; }
function eventSeqKey(userId) { return `ms:metrics:events:seq:${userId}`; }
function eventZKey(userId) { return `ms:metrics:events:z:${userId}`; }
function eventHashKey(userId) { return `ms:metrics:events:h:${userId}`; }
function monthlySentKey(userId) { return `ms:metrics:monthly:sent:${userId}`; }
function monthlyErrorKey(userId) { return `ms:metrics:monthly:error:${userId}`; }
function contactStatsKey(userId) { return `ms:metrics:contact:stats:${userId}`; }
...
async function listCampaigns(userId, opts = {}) {
  ...
  const ids = await r.zrevrange(userCampaignsKey(userId), 0, -1);
  const total = ids.length;

  const offset = (page - 1) * pageSize;
  const pagedIds = ids.slice(offset, offset + pageSize);

  const items = [];
  for (const id of pagedIds) {
    const raw = await r.get(campaignDataKey(id));
    const campaign = safeJsonParse(raw);
